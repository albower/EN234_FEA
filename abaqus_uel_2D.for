...
!=========================== ABAQUS format user element subroutine ===================
    ...
    !
      ...
      double precision  ::  dxdxi(2,2), dxdxi1(2,2)          ! Derivative of spatial coords wrt normalized coords
    !
      ...
      double precision  ::  thk, wid, lbar, dv(2), sn1, sn2, cs1, cs2
      double precision  ::  scoords(2,4), Tmat(8,6), Rmat(2,3)
      double precision  ::  strainhat(2), stresshat(2)
      double precision  ::  RBT(2,6), stavar(3)
    ...
    !     PROPS(2)         Poisson's ratio
      ...
      if (NNODE == 2) n_points = 5              ! 1-D element
    ...
    ! Write your code for a 2D element below
      ...
      lbar = norm2(coords(1:2,2)-coords(1:2,1))
      dv(1:2) = (coords(1:2,2)-coords(1:2,1))/lbar
      sn1 =  dv(2)*cos(U(3)) + dv(1)*sin(U(3))
      cs1 =  dv(1)*cos(U(3)) - dv(2)*sin(U(3))
      sn2 =  dv(2)*cos(U(6)) + dv(1)*sin(U(6))
      cs2 =  dv(1)*cos(U(6)) - dv(2)*sin(U(6))
      scoords(1,1) = coords(1,1) + thk/2*sn1
      scoords(2,1) = coords(2,1) - thk/2*cs1
      scoords(1,2) = coords(1,2) + thk/2*sn2
      scoords(2,2) = coords(2,2) - thk/2*cs2
      scoords(1,3) = coords(1,2) - thk/2*sn2
      scoords(2,3) = coords(2,2) + thk/2*cs2
      scoords(1,4) = coords(1,1) - thk/2*sn1
      scoords(2,4) = coords(2,1) + thk/2*cs1
      
      Tmat = 0.d0
      Tmat(1,1) = 1.d0
      Tmat(1,3) = coords(2,1) - scoords(2,1)
      Tmat(2,2) = 1.d0
      Tmat(2,3) = -coords(1,1) + scoords(1,1)
      Tmat(3,4) = 1.d0
      Tmat(3,6) = coords(2,2) - scoords(2,2)
      Tmat(4,5) = 1.d0
      Tmat(4,6) = coords(1,2) - scoords(1,2)
      Tmat(5,4) = 1.d0
      Tmat(5,6) = coords(2,2) - scoords(2,3)
      Tmat(6,5) = 1.d0
      Tmat(6,6) = coords(1,2) - scoords(1,3)
      Tmat(7,1) = 1.d0
      Tmat(7,3) = coords(2,1) - scoords(2,4)
      Tmat(8,2) = 1.d0
      Tmat(8,3) = -coords(1,1) + scoords(1,4)

    !     --  Loop over integration points
      do kint = 1, n_points
        call abq_UEL_2D_shapefunctions(xi(1:2,kint),4,N,dNdxi)
        dxdxi = matmul(scoords(1:2,1:4),dNdxi(1:4,1:2))
        dxdxi1(1:2,1) = dxdxi(1:2,1)/norm2(dxdxi(1:2,1))
        dxdxi1(1:2,2) = dxdxi(1:2,2)/norm2(dxdxi(1:2,2))
        
        Rmat(1,1) = dxdxi1(1,1)*dxdxi1(1,1)
        Rmat(1,2) = dxdxi1(1,2)*dxdxi1(1,2)
        Rmat(1,3) = dxdxi1(1,1)*dxdxi1(1,2)
        Rmat(2,1) = 2*dxdxi1(1,1)*dxdxi1(2,1)
        Rmat(2,2) = 2*dxdxi1(1,2)*dxdxi1(2,2)
        Rmat(2,3) = dxdxi1(1,1)*dxdxi1(2,2) + dxdxi1(1,2)*dxdxi1(2,1)
        ...
        strain(1:3) = matmul(B(1:3,1:8),matmul(Tmat(1:8,1:6),U(1:6)))
        strainhat = matmul(Rmat(1:2,1:3),strain(1:3))
        stresshat = matmul(D,strainhat)
        
        RBT = matmul(Rmat(1:2,1:3),matmul(B(1:3,1:8),Tmat(1:8,1:6)))
        RHS(1:3*NNODE,1) = RHS(1:3*NNODE,1)
     1  - wid*lbar*thk/2*matmul(transpose(RBT),stresshat(1:2))*w(kint)

        AMATRX(1:3*NNODE,1:3*NNODE) = AMATRX(1:3*NNODE,1:3*NNODE)
     1  + wid*lbar*thk/2*matmul(transpose(RBT),matmul(D,RBT))*w(kint)
        
        stavar(1) = stavar(1) + wid*thk/2*stresshat(1)*w(kint)
        stavar(2) = stavar(2) + wid*thk/2*stresshat(2)*w(kint)
        stavar(3) = stavar(3) + wid*thk*thk/4
     1                        *stresshat(1)*xi(2,kint)*w(kint)

      end do
      
      SVARS(1:3) = stavar(1:3)
