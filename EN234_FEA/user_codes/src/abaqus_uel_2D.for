...
!=========================== ABAQUS format user element subroutine ===================
    ...
    !     PROPS(2)         Poisson's ratio
      ...
      if (JTYPE==2) then
        ...
        det0 = dxdxi(1,1)*dxdxi(2,2)-dxdxi(1,2)*dxdxi(2,1)
        ksize = 2*NNODE + 4
      endif
    ...
    !     --  Loop over integration points
      do kint = 1, n_points
        ...
        if (JTYPE==2) then
          B(1,2*NNODE+1:2*NNODE+3:2) =
     1                 det0/determinant*xi(1:2,kint)*dxidx(1:2,1)
          B(2,2*NNODE+2:2*NNODE+4:2) =
     1                 det0/determinant*xi(1:2,kint)*dxidx(1:2,2)
          B(4,2*NNODE+1:2*NNODE+3:2) =
     1                 det0/determinant*xi(1:2,kint)*dxidx(1:2,2)
          B(4,2*NNODE+2:2*NNODE+4:2) =
     1                 det0/determinant*xi(1:2,kint)*dxidx(1:2,1)
        endif

        ktemp(1:ksize,1:ksize) = ktemp(1:ksize,1:ksize)
     1  + matmul(transpose(B(1:4,1:ksize)),matmul(D,B(1:4,1:ksize)))
...
!       Incompatible mode elements
     ...
     1                 det0/determinant*xi(1:2,kint)*dxidx(1:2,2)
          B(4,2*NNODE+2:2*NNODE+4:2) =
     1                 det0/determinant*xi(1:2,kint)*dxidx(1:2,1)
          strain = strain + matmul(B(1:4,2*NNODE+1:2*NNODE+4),alpha)
        endif

        stress = matmul(D,strain)

        rhs_temp(1:ksize) = rhs_temp(1:ksize)
     1  - matmul(transpose(B(1:4,1:ksize)),stress(1:4))
     2                                             *w(kint)*determinant
     ...
     2                 matmul(kaainv,rhs_temp(2*NNODE+1:2*NNODE+4)))
    ...
    !
     ...
     1                                  face_node_list,nfacenodes)
        ...
        do kint = 1,n_points
            ...
            if (nfacenodes==2) then
                N1(1) = 0.5d0*(1.d0-xi1(1))
                N1(2) = 0.5d0*(1.d0+xi1(1))
                dN1dxi(1) = -0.5d0
                dN1dxi(2) =  0.5d0
            else if (nfacenodes==3) then
                N1(1) = -0.5*xi1(1)*(1.-xi1(1))
                N1(2) =  0.5*xi1(1)*(1.+xi1(1))
                N1(3) = (1.-xi1(1))*(1.+xi1(1))
                dN1dxi(1) = -0.5+xi1(1)
                dN1dxi(2) =  0.5+xi1(1)
                dN1dxi(3) = -2.d0*xi1(1)
            endif
